...
@ray.remote
def runSim(numMax, grid_input, p, alpha, pair_frac, actual_N):
    ...
    runs = ray.get([runRoute.remote(graph_key, actual_N, numMax) for i in range(int(pair_frac * actual_N))])
    print(runs[1:10])
...
def runSimulation(N=100, dim=1, num_graph_gen=25, pair_frac=0.01,
    ...
    results = ray.get([runSim.remote(numMax, grid_input, p, alpha, pair_frac, actual_N) for i in range(num_graph_gen)])
